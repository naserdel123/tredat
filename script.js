...
function createTradeCard(post, index) {
    ...
    // TikTok أيقونة دائماً موجودة لأنها إجبارية
    const tiktokHtml = `<a href="${post.tiktokLink}" target="_blank" class="tiktok-btn" title="زيارة حساب TikTok"></a>`;
    ...
    div.innerHTML = `
        ...
        <div class="card-body">
            ...
            <div class="trade-for-image">
                <img src="${post.tradeImage}" alt="المقابل" class="trade-image-small">
            </div>
...
function createTrade(e) {
    ...
    if (!itemImageInput.files || !itemImageInput.files[0]) {
        alert('يرجى اختيار صورة العرض');
        document.querySelectorAll('.upload-box')[0].classList.add('error');
        setTimeout(() => {
            document.querySelectorAll('.upload-box')[0].classList.remove('error');
        }, 1000);
        return;
    }
    
    if (!tradeImageInput.files || !tradeImageInput.files[0]) {
        alert('يرجى اختيار صورة المقابل');
        document.querySelectorAll('.upload-box')[1].classList.add('error');
        setTimeout(() => {
            document.querySelectorAll('.upload-box')[1].classList.remove('error');
        }, 1000);
        return;
    }
    ...
    if (!tiktokLink || !tiktokLink.includes('tiktok.com')) {
        alert('يرجى إدخال رابط حساب TikTok صحيح');
        document.getElementById('tiktok-link').focus();
        return;
    }
    ...
    reader1.onload = function(e) {
        ...
        reader2.readAsDataURL(tradeImageInput.files[0]);
    };
    reader1.readAsDataURL(itemImageInput.files[0]);
}
